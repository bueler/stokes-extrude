...
def _setup_physics_2d_iceslab(se, L, H, alpha):
    ...
    C = (2.0 / (nglen + 1.0)) * (rho * g * sin(alpha) / B3) ** nglen
    uin = as_vector([C * (H ** (nglen + 1.0) - (H - z) ** (nglen + 1.0)), 0.0])
    se.dirichlet((1,), uin)
    stressout = as_vector(
        [-rho * g * cos(alpha) * (H - z), rho * g * sin(alpha) * (H - z)]
    )
    ...
    rr = 1.0 / nglen - 1.0
    F = (
        inner(B3 * Du2 ** (rr / 2.0) * se.D(u), se.D(v))
        - p * div(v)
        - div(u) * q
        - inner(f_body, v)
    ) * dx(degree=4)
