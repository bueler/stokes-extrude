...
def _linear_stokes_F(se, f_body=None):
    assert f_body is not None
    u, p = split(se.up)
    v, q = TestFunctions(se.Z)
    F = (
        inner(2.0 * se.nu * se.D(u), se.D(v))
        - p * div(v)
        - q * div(u)
        - inner(f_body, v)
    ) * dx
...
def test_stokes_3d_hydrostatic_mumps():
    ...
    F = _linear_stokes_F(se, f_body=Constant((0.0, 0.0, -1.0)))
...
def _setup_physics_2d_slab(se, L, H):
    ...
    CC = rho0 * g
    F = _linear_stokes_F(se, f_body=Constant((CC * sin(alpha), -CC * cos(alpha))))
    se.dirichlet(("bottom",), Constant((0.0, 0.0)))
    C0 = CC * sin(alpha) / nu0
    _, z = SpatialCoordinate(se.mesh)
    u_in = as_vector([C0 * z * (H - z / 2), Constant(0.0)])
    se.dirichlet((1,), u_in)
    stress_out = as_vector([-CC * cos(alpha) * (H - z), CC * sin(alpha) * (H - z)])
...
def test_stokes_2d_slab_mumps():
    ...
    uexact, pexact = _exact_2d_slab(
        se.mesh, u.function_space(), p.function_space(), L, H
    )
...
def test_stokes_pinch_mumps():
    ...
    g, rho0, nu0 = 9.8, 1.0, 1.0
    se.viscosity_constant(nu0)
    F = _linear_stokes_F(se, f_body=Constant((0.0, -rho0 * g)))
    se.dirichlet(("bottom",), Constant((0.0, 0.0)))
    params = SolverParams["newton"]
    params.update(SolverParams["mumps"])
    # params['ksp_converged_reason'] = None
    ...
    assert (
        se.solver.snes.ksp.getConvergedReason()
...
def test_stokes_pinch_gmg():
    ...
    for j in range(levs):
        ...
        s[j] = Function(P1b).interpolate(
            conditional(abs(xc) < 0.5, sqrt(0.25 - xc * xc), hmin)
        )
    se.reset_elevations(0.0, s)
    if False:
